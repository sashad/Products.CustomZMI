    ...
    window.editor.on('focus', function() {
        if (form) {
            ...
            if (pos) {
                ...
                dom.setCssClass(window.editor.container, "fullScreen", (fs === 'true'));
                window.editor.moveCursorTo(parseInt(rc[0]), parseInt(rc[1]));
                var toScrollToLine = true;
                window.editor.renderer.on('afterRender', function() {
                    if (toScrollToLine) {
                        window.editor.scrollToLine(parseInt(rc[0]), true);
